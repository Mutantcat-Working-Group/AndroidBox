...
varrun="/run/androidbox-lxc"
varlib="/var/lib"
net_link_key="lxc.net.0.link"
case "$(lxc-info --version)" in [012].*) net_link_key="lxc.network.link" ;; esac
vnic=$(awk "\$1 == \"$net_link_key\" {print \$3}" /var/lib/androidbox/lxc/androidbox/config)
: ${vnic:=androidbox0}

if [ "$vnic" != "androidbox0" ]; then
    echo "vnic is $vnic, bailing out"
    exit 0
else 
    echo "vnic is androidbox0"
fi
...
start() {
    ...
    [ ! -f "${varrun}/network_up" ] || { echo "androidbox-net is already running"; exit 0; }
    ...
    cleanup() {
        set +e
        if [ "$FAILED" = "1" ]; then
            echo "Failed to setup androidbox-net." >&2
            stop force
            exit 1
        fi
    }
...
stop() {
    ...
    [ -f "${varrun}/network_up" ] || [ "$1" = "force" ] || { echo "androidbox-net isn't running"; exit 1; }
