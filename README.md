...
当前版本 `1.0.20261003`。安装包内置 Python、Qt、noVNC、QEMU、ADB 与整套 Android 系统镜像；Windows 安装器把镜像压缩后追加在自身上，首次启动自动展开。
...
推送 `v*` 版本标签（例如 `v1.0.20261003`，标签需与源码版本一致）即由 GitHub Actions 自动构建五个安装包并联编 Release；在 Actions 页面手动运行只产出 CI 制品，不发布版本。已发布的 Release 不会被重复运行覆盖。
